"""Strict argument grammar for provider custom-command input.

Provider commands pass user text to the model, not directly to a process.
...
class CommandArgumentError(ValueError):
    """Command input is outside the documented capability grammar."""
...
def _tokens(raw):
    if not isinstance(raw, str):
        raise CommandArgumentError("arguments must be text")
    if len(raw) > 4096 or any(ord(character) < 32 or ord(character) == 127 for character in raw):
        raise CommandArgumentError("arguments must not contain control characters")
    if any(character in _METACHARACTERS for character in raw):
        raise CommandArgumentError("shell metacharacters are not allowed")
    try:
        tokens = shlex.split(raw, posix=True)
    except ValueError as error:
        raise CommandArgumentError("arguments must use balanced shell-style quotes: {0}".format(error))
    if len(tokens) > 32 or any(not token or len(token) > 512 for token in tokens):
        raise CommandArgumentError("arguments exceed the supported command grammar")
    return tokens


def _value(tokens, index, flag):
    if index + 1 >= len(tokens) or tokens[index + 1].startswith("--"):
        raise CommandArgumentError("{0} requires a value".format(flag))
    return tokens[index + 1]


def _path(value, label):
    if not isinstance(value, str) or not value or value.startswith("~") or "\\" in value:
        raise CommandArgumentError("{0} must be a portable non-empty path".format(label))
    path = Path(value)
    if any(part in {"", ".", ".."} for part in path.parts):
        raise CommandArgumentError("{0} must not traverse parent directories".format(label))
    return value


def _model(value):
    if not _MODEL.fullmatch(value):
        raise CommandArgumentError("model must be a publisher/model identifier")
    return value


def _integer(value, label, minimum, maximum):
    if not re.fullmatch(r"[0-9]+", value):
        raise CommandArgumentError("{0} must be an integer".format(label))
    number = int(value)
    if number < minimum or number > maximum:
        raise CommandArgumentError("{0} must be between {1} and {2}".format(label, minimum, maximum))
    return str(number)


def _decimal(value, label, minimum, maximum):
    if not re.fullmatch(r"(?:[0-9]+(?:\.[0-9]+)?|\.[0-9]+)", value):
        raise CommandArgumentError("{0} must be a decimal number".format(label))
    number = float(value)
    if number < minimum or number > maximum:
        raise CommandArgumentError("{0} must be between {1} and {2}".format(label, minimum, maximum))
    return value
...
def _endpoint(value):
    try:
        ...
        raise CommandArgumentError("endpoint is not a valid local HTTP URL") from error
    if (
        ...
        raise CommandArgumentError("endpoint must be a credential-free local HTTP URL")
    if port is not None and not 1 <= port <= 65535:
        raise CommandArgumentError("endpoint port must be between 1 and 65535")
    return value
...
def _parse_scout(tokens):
    ...
    while index < len(tokens):
        ...
        if flag in booleans:
            if flag in seen:
                raise CommandArgumentError("duplicate flag: {0}".format(flag))
            if flag in {"--exclude-gated", "--include-gated"} and {"--exclude-gated", "--include-gated"} & seen:
                raise CommandArgumentError("gated-repository flags are mutually exclusive")
            seen.add(flag)
            argv.append(flag)
            index += 1
        elif flag in {"--refresh", "--offline"}:
            if cache is not None:
                raise CommandArgumentError("--refresh and --offline are mutually exclusive")
            cache = flag
            argv.append(flag)
            index += 1
            ...
            if not _SAFE_NAME.fullmatch(value):
                raise CommandArgumentError("{0} must be a safe identifier".format(flag))
            ...
            if value is None:
                raise CommandArgumentError("invalid {0} value".format(values[flag][1]))
            seen.add(flag)
            argv.extend([flag, value])
            index += 2
        else:
            raise CommandArgumentError("unsupported Scout argument: {0}".format(flag))
    return argv


def _parse_adopt(tokens):
    if not tokens or tokens[0] not in {"start", "resume", "status"}:
        raise CommandArgumentError("Adopt requires start, resume, or status")
    ...
    while index < len(tokens):
        ...
        if flag == "--state":
            if state is not None:
                raise CommandArgumentError("duplicate flag: --state")
            ...
            if role not in _ROLES:
                raise CommandArgumentError("unknown adoption role")
            ...
            raise CommandArgumentError("unsupported Adopt argument: {0}".format(flag))
    if state is None:
        raise CommandArgumentError("Adopt requires an explicit --state path")
    return argv


def _parse_wire(tokens):
    if not tokens or tokens[0] not in {"render", "apply", "status", "rollback"}:
        raise CommandArgumentError("Wire requires render, apply, status, or rollback")
    action, argv, index = tokens[0], ["wire", tokens[0]], 1
    if action in {"status", "rollback"}:
        if index >= len(tokens):
            raise CommandArgumentError("Wire {0} requires a receipt path".format(action))
        ...
        if index >= len(tokens):
            raise CommandArgumentError("Wire {0} requires a model".format(action))
    ...
    while index < len(tokens):
        ...
        if flag not in allowed:
            raise CommandArgumentError("unsupported Wire argument: {0}".format(flag))
        if flag in argv:
            raise CommandArgumentError("duplicate flag: {0}".format(flag))
...
def _parse_bench(tokens):
    ...
    while index < len(tokens):
        ...
        if flag == "--json":
            if flag in seen:
                raise CommandArgumentError("duplicate flag: --json")
            seen.add(flag)
            argv.append(flag)
            index += 1
        elif flag in values:
            ...
            raise CommandArgumentError("unsupported Bench argument: {0}".format(flag))
    for required in ("--repo", "--runtime"):
        if required not in seen:
            raise CommandArgumentError("Bench run requires {0}".format(required))
    return argv
...
def _parse_bench_aggregate(tokens):
    ...
    if "--exports" not in seen:
        raise CommandArgumentError("Bench aggregate requires --exports")
    return argv


def _parse_doctor(tokens):
    if not tokens or tokens[0] != "models":
        raise CommandArgumentError("Doctor requires the models action")
    ...
    while index < len(tokens):
        ...
        if flag == "--json":
            ...
            if flag in seen:
                raise CommandArgumentError("duplicate flag: --hf-cache")
            seen.add(flag)
            argv.extend([flag, _path(_value(tokens, index, flag), "hf-cache")])
            index += 2
        else:
            raise CommandArgumentError("unsupported Doctor argument: {0}".format(flag))
    return argv


def _parse_watch(tokens):
    if not tokens or tokens[0] not in {"snapshot", "diff"}:
        raise CommandArgumentError("Watch requires the snapshot or diff action")
    ...
    while index < len(tokens):
        ...
        if flag == "--json":
            ...
            if flag in seen:
                raise CommandArgumentError("duplicate flag: --state-dir")
            seen.add(flag)
            argv.extend([flag, _path(_value(tokens, index, flag), "state-dir")])
            index += 2
        elif flag == "--limit":
            if flag in seen:
                raise CommandArgumentError("duplicate flag: --limit")
            seen.add(flag)
            argv.extend([flag, _integer(_value(tokens, index, flag), "limit", 1, 50)])
            index += 2
        else:
            raise CommandArgumentError("unsupported Watch argument: {0}".format(flag))
    return argv


def _fleet_assignment(value, label):
    if "=" not in value:
        raise CommandArgumentError("{0} must use role=value form".format(label))
    role, target = value.split("=", 1)
    if role not in _ROLES:
        raise CommandArgumentError("{0} must name a canonical role".format(label))
    return value


def _parse_fleet(tokens):
    if not tokens or tokens[0] not in {"render", "apply"}:
        raise CommandArgumentError("Fleet requires the render or apply action")
    ...
    while index < len(tokens):
        ...
        if flag in {"--json", "--confirm", "--allow-missing"}:
            ...
            if value.split("=", 1)[1] not in ("mlx_lm", "mlx-vlm"):
                raise CommandArgumentError("--runtime-map must target mlx_lm or mlx-vlm")
            argv.extend([flag, value])
            index += 2
        elif flag == "--path":
            if flag in seen:
                raise CommandArgumentError("duplicate flag: --path")
            seen.add(flag)
            ...
            raise CommandArgumentError("unsupported Fleet argument: {0}".format(flag))
    if not path_seen:
        raise CommandArgumentError("Fleet {0} requires --path".format(action))
    return argv


def parse_command_arguments(capability, raw):
    ...
    raise CommandArgumentError("unknown command capability")
...
def main(argv=None):
    ...
    try:
        print(json.dumps({"argv": parse_command_arguments(arguments.capability, arguments.raw)}))
