...
`capability` set to `watch` and `arguments` set to the exact text inside
...
Watch writes only its own state file and must not download model weights or change configuration.
