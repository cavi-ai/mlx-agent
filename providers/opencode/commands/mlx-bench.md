...
Load and follow the bundled `mlx-bench` skill before acting. The block
...
that tool once with `capability` set to `bench` and `arguments` set to
...
Run only the validated measurement operation against an already-served model. Do not start servers, download model weights, or change configuration.
